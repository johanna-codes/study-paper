...
j=1;
  for pe_ts= 1: n_peo
      ...
      load_svm_model =strcat( './svm_models/projRGB_svm_run_', int2str(pe_ts), '_delta', num2str(delta),'.mat');
      load(load_svm_model); %loading model and X_train

      for act_ts = 1:n_actions
          
          real_labels(j) = act_ts;
          name_load_gp = strcat( load_sub_path, '/grass_pt_', all_people(pe_ts), '_', actions(act), '_dim', int2str(dim), '.h5');     
          hinfo = hdf5info( char(name_load_gp) );
          one_video = hdf5read(hinfo.GroupHierarchy.Datasets(1));
          X_test(:,:,1) = one_video;
          
          K_test = compute_proj_kernel_svm(X_test,X_train, PROJECTION_RBF_KERNEL,gamma);
